    ...
    for a in area:
        ...
        for car in vehicles:
            if car.is_available == True:
                vehicle_dictionary = {'name':car.car_name, 'color':car.color, 'id':car.id , 'cost':car.cost_per_hour, 'pincode':car.area.pincode, 'capacity':car.capacity, 'description':car.description}
...
def rent_vehicle(request):
    ...
    cost_per_hour = int(vehicle.cost_per_hour)
    return render(request, 'customer/confirmation.html', {'vehicle':vehicle, 'cost_per_hour':cost_per_hour})
...
def confirm(request):
    ...
    if vehicle.is_available:
        ...
        rent = (int(vehicle.cost_per_hour))*(int(hours))
        ...
        car_dealer.save()
        try:
            order = Orders(vehicle = vehicle, car_dealer = car_dealer, user = user, rent=rent, hours=hours)
            order.save()
        except:
            order = Orders.objects.get(vehicle = vehicle, car_dealer = car_dealer, user = user, rent=rent, hours=hours)
    ...
    if orders is not None:
        for o in orders:
            if o.is_complete == False:
                order_dictionary = {'id':o.id,'rent':o.rent, 'vehicle':o.vehicle, 'hours':o.hours, 'car_dealer':o.car_dealer}
